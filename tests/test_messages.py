def test_messages(connector, patient, practitioner, careplan, activity):
    ...
    def get_num_messages(patient):
        return len(list(parse(
            connector.messages(patient=patient, summary=True))))

    # We use the careplan fixture so we know there's at least one message in
    # the mailbox.
    num_messages_before = get_num_messages(careplan.patient)
    xml = generate(
        connector.domain, activity, careplan, practitioner)
    connector.post_message(xml)
    num_messages_after = get_num_messages(careplan.patient)
...
def test_messages_for_patient(
    ...
    from koppeltaal.feed import parse
    messages = parse(
        connector.messages(patient=careplan.patient, summary=True))
...
def test_messages_for_status(connector, patient, careplan, careplan_on_server):
    ...
    messages2 = list(parse(
        connector.messages(patient=careplan.patient, processing_status="New")))
    ...
    messages2 = list(parse(
        connector.messages(
            patient=careplan.patient, processing_status="Claimed")))
